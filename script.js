...
// --- EURIBOR ---
const euriborValor=document.getElementById("euriborValor");
euriborValor.innerText="4,15%"; // Se puede actualizar automáticamente desde API si se desea
...
const resultadosDiv=document.querySelector("#calculadora .cards");
...
function generarTabla(){
  ...
  for(let i=1;i<=n;i++){
    ...
    saldo-=capitalMes;
    tbody.innerHTML+=`<tr>
<td>${i}</td><td>${formatMoney(cuota)}</td><td>${formatMoney(interesMes)}</td><td>${formatMoney(capitalMes)}</td><td>${formatMoney(Math.max(saldo,0))}</td>
</tr>`;
  }
}
...
[perfilEdad1,perfilEdad2,perfilSalario1,perfilSalario2,perfilPagas,perfilAhorros,perfilDeuda,perfilOtroIngreso,yaTieneVivienda,perfilPrecio,perfilTipoVivienda,perfilComunidad,perfilPlazo,perfilInteres].forEach(el=>el.addEventListener("input",calcularPerfil));
...
function calcularPerfil(){
  ...
  // Plazo máximo según edad y tipo de vivienda
  let plazoMax=Math.min(30,75-maxEdad);
  if(perfilTipoVivienda.value==="segunda"||perfilTipoVivienda.value==="local"){plazoMax=15;}
  ...
  if(yaTieneVivienda.checked){
    ...
    capitalPosible=precio*gastos>0?Math.min(capitalPosible,precio*0.7):capitalPosible; // Segunda o local al 70%
    if(perfilTipoVivienda.value==="segunda"){alert("Segunda residencia: Financiación máxima 70%, considerar ahorro adicional");}
    if(perfilTipoVivienda.value==="local"){alert("Local comercial: Financiación máxima 70%, considerar ahorro adicional");}
  }
